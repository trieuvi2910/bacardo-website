...
export async function GET(request: NextRequest) {
  try {
    ...
    // Sort by upload date (newest first) and take only 5
    publicImages.sort((a, b) => new Date(b.uploadedAt).getTime() - new Date(a.uploadedAt).getTime())
    const latestPublicImages = publicImages.slice(0, 5)

    return NextResponse.json({
      images: latestPublicImages,
      total: latestPublicImages.length,
      message: `Found ${latestPublicImages.length} latest public images`
    })

  } catch (error) {
    console.error('Public gallery read error:', error)
    return NextResponse.json(
      { error: 'Failed to read public gallery images' },
      { status: 500 }
    )
  }
}
